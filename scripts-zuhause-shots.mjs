...
await page.waitForFunction(() => {
  ...
  return texts.some((t) => /المنزل|Home|Zuhause/.test(t) && !/🔒/.test(t));
}, { timeout: 5000 }).catch(() => null);

await new Promise((r) => setTimeout(r, 400));
await shot(page, 'zuhause-topics.png');
...
await page.evaluate(() => {
  ...
  const b = btns.find((el) => {
    ...
    return /المنزل|Home|Zuhause/.test(t) && !/اختر|Choose|Spiel|Start|رجوع|Back|Zurück/.test(t);
  });
  b?.click();
});
...
await new Promise((r) => setTimeout(r, 500));
await shot(page, 'zuhause-picture-match.png');
