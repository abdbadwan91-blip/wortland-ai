...
await page.waitForFunction(() => {
  ...
  return texts.some((t) => /مدرسة|School|Schule/.test(t) && !/🔒/.test(t));
...
await page.evaluate(() => {
  ...
  const b = btns.find((el) => {
    ...
    return /مدرسة|School|Schule/.test(t) && !/اختر|Choose|Spiel|Start|رجوع|Back|Zurück/.test(t);
  });
  b?.click();
});
await new Promise((r) => setTimeout(r, 400));
await shot(page, 'schule-topics.png');
...
await new Promise((r) => setTimeout(r, 500));
await shot(page, 'schule-picture-match.png');
