...
export function SpeedRoundResultsScreen({
  ...
  const rewards = calcRoundRewards(correct, roundSize, { badgeId: 'rennwolf' });
  ...
  return (
    <div className={`screen fade-in ${styles.wrap}`} data-speed-results>
      <div className={styles.hero}>
        <div
          className={styles.stars}
          aria-label={t('sr.stars', { n: rewards.stars })}
        >
          {stars.map((on, i) => (
            <span
              key={i}
        ...
        <h1 className={styles.title}>{t('sr.resultsTitle')}</h1>
        <p className={styles.score}>
          {t('sr.score', { correct, total: roundSize })}
        </p>
        ...
        {rewards.badgeId && (
          <div className={styles.badge} data-badge="rennwolf">
            ⚡ {t('sr.badge.rennwolf')}
          </div>
        )}
        <p
        ...
        >
          {rewards.mapAdvance
            ? t('sr.mapAdvanced')
            : t('sr.mapNeedMore', { n: MAP_PASS_SCORE })}
        </p>
      </div>
      ...
      <div className={styles.actions}>
        ...
        <button
          type="button"
          className="btn-blue"
          onClick={() => handleNextChallenge({ setScreen, setSelectedMode, setSelectedLevel }, { currentMode: 'speed', level: selectedLevel || profile.level || 1, passed: Boolean(rewards?.mapAdvance), onRetry: onPlayAgain })}
          data-next-challenge
        >
          {t('sr.nextChallenge')}
        </button>
