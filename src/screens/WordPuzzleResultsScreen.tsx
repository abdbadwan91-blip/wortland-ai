...
export function WordPuzzleResultsScreen({
  correct,
  sessionXp,
  onPlayAgain,
}: Props) {
  ...
  const rewards = calcRoundRewards(correct, roundSize, { badgeId: 'wortzauberer' });
  ...
  return (
    <div className={`screen fade-in ${styles.wrap}`} data-puzzle-results>
      <div className={styles.hero}>
        <div
          className={styles.stars}
          aria-label={t('wp.stars', { n: rewards.stars })}
        >
          {stars.map((on, i) => (
            <span
              key={i}
        ...
        <h1 className={styles.title}>{t('wp.resultsTitle')}</h1>
        <p className={styles.score}>
          {t('wp.score', { correct, total: roundSize })}
        </p>
        ...
        {rewards.badgeId && (
          <div className={styles.badge} data-badge="wortzauberer">
            🔠 {t('wp.badge.wortzauberer')}
          </div>
        )}
        <p
          className={
        ...
        >
          {rewards.mapAdvance
            ? t('wp.mapAdvanced')
            : t('wp.mapNeedMore', { n: MAP_PASS_SCORE })}
        </p>
      </div>
      ...
      <div className={styles.actions}>
        ...
        <button
          type="button"
          className="btn-blue"
          onClick={() => handleNextChallenge({ setScreen, setSelectedMode, setSelectedLevel }, { currentMode: 'puzzle', level: selectedLevel || profile.level || 1, passed: Boolean(rewards?.mapAdvance), onRetry: onPlayAgain })}
          data-next-challenge
        >
          {t('wp.nextChallenge')}
        </button>
