...
export function PictureMatchResultsScreen({
  correct,
  sessionXp,
  onPlayAgain,
}: Props) {
  ...
  return (
    <div className={`screen fade-in ${styles.wrap}`} data-picture-results>
      <div className={styles.hero}>
        <div
          className={styles.stars}
          aria-label={t('pm.stars', { n: rewards.stars })}
        >
          {stars.map((on, i) => (
            <span
              key={i}
        ...
        <h1 className={styles.title}>{t('pm.resultsTitle')}</h1>
        <p className={styles.score}>
          {t('pm.score', { correct, total: roundSize })}
        </p>
        ...
        >
          {rewards.mapAdvance
            ? t('pm.mapAdvanced')
            : t('pm.mapNeedMore', { n: MAP_PASS_SCORE })}
        </p>
        <p className={styles.transLabel}>
          {t('pm.translationLang')}: {profile.translationLanguage.toUpperCase()}
        </p>
      </div>
      ...
      <div className={styles.actions}>
        ...
        <button
          type="button"
          className="btn-blue"
          onClick={() => handleNextChallenge({ setScreen, setSelectedMode, setSelectedLevel }, { currentMode: 'picture', level: selectedLevel || profile.level || 1, passed: Boolean(rewards?.mapAdvance), onRetry: onPlayAgain })}
          data-next-challenge
        >
          {t('pm.nextChallenge')}
        </button>
