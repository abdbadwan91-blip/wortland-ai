...
export function ListeningHuntResultsScreen({
  correct,
  sessionXp,
  onPlayAgain,
}: Props) {
  ...
  return (
    <div className={`screen fade-in ${styles.wrap}`} data-listening-results>
      <div className={styles.hero}>
        <div
          className={styles.stars}
          aria-label={t('lh.stars', { n: rewards.stars })}
        >
          {stars.map((on, i) => (
            <span
              key={i}
        ...
        <h1 className={styles.title}>{t('lh.resultsTitle')}</h1>
        <p className={styles.score}>
          {t('lh.score', { correct, total: roundSize })}
        </p>
        ...
        {rewards.badgeId && (
          <div className={styles.badge} data-badge="ohrenjaeger">
            🎧 {t('lh.badge.ohrenjaeger')}
          </div>
        )}
        <p
          className={
        ...
        >
          {rewards.mapAdvance
            ? t('lh.mapAdvanced')
            : t('lh.mapNeedMore', { n: MAP_PASS_SCORE })}
        </p>
      </div>
