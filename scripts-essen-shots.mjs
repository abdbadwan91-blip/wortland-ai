...
// Topic picker — wait and shoot with Essen unlocked
await page.waitForFunction(() => {
  const texts = [...document.querySelectorAll('button')].map((b) => b.textContent || '');
  return texts.some((t) => /طعام|Food|Essen/.test(t) && !/🔒/.test(t));
}, { timeout: 5000 }).catch(() => null);

await new Promise((r) => setTimeout(r, 400));
await shot(page, 'essen-topics.png');
...
await new Promise((r) => setTimeout(r, 500));
await shot(page, 'essen-picture-match.png');
