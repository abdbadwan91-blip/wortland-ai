...
await page.waitForFunction(() => {
  ...
  return texts.some((t) => /أرقام|Numbers|Zahlen/.test(t) && !/🔒/.test(t));
...
await page.evaluate(() => {
  ...
  const b = btns.find((el) => {
    ...
    return /أرقام|Numbers|Zahlen/.test(t) && !/اختر|Choose|Spiel|Start|رجوع|Back|Zurück/.test(t);
  });
  b?.click();
});
await new Promise((r) => setTimeout(r, 400));
await shot(page, 'zahlen-topics.png');
...
await new Promise((r) => setTimeout(r, 500));
await shot(page, 'zahlen-picture-match.png');
