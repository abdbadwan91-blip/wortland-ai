...
await page.waitForFunction(() => {
  ...
  return texts.some((t) => /ملابس|Clothes|Kleidung/.test(t) && !/🔒/.test(t));
...
await page.evaluate(() => {
  ...
  const b = btns.find((el) => {
    ...
    return /ملابس|Clothes|Kleidung/.test(t) && !/اختر|Choose|Spiel|Start|رجوع|Back|Zurück/.test(t);
  });
  b?.click();
});
await new Promise((r) => setTimeout(r, 400));
await shot(page, 'kleidung-topics.png');
...
await new Promise((r) => setTimeout(r, 500));
await shot(page, 'kleidung-picture-match.png');
