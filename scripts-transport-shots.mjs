...
await page.waitForFunction(() => {
  ...
  return texts.some((t) => /مواصلات|Transport|Verkehr/.test(t) && !/🔒/.test(t));
...
await page.evaluate(() => {
  ...
  const b = btns.find((el) => {
    ...
    return /مواصلات|Transport|Verkehr/.test(t) && !/اختر|Choose|Spiel|Start|رجوع|Back|Zurück/.test(t);
  });
  b?.click();
});
await new Promise((r) => setTimeout(r, 400));
await shot(page, 'transport-topics.png');
...
await new Promise((r) => setTimeout(r, 500));
await shot(page, 'transport-picture-match.png');
