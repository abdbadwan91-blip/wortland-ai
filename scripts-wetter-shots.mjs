...
await page.waitForFunction(() => {
  ...
  return texts.some((t) => /طقس|Weather|Wetter/.test(t) && !/🔒/.test(t));
...
await page.evaluate(() => {
  ...
  const b = btns.find((el) => {
    ...
    return /طقس|Weather|Wetter/.test(t) && !/اختر|Choose|Spiel|Start|رجوع|Back|Zurück/.test(t);
  });
  b?.click();
});
await new Promise((r) => setTimeout(r, 400));
await shot(page, 'wetter-topics.png');
...
await new Promise((r) => setTimeout(r, 500));
await shot(page, 'wetter-picture-match.png');
