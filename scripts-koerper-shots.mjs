...
await page.waitForFunction(() => {
  ...
  return texts.some((t) => /جسم|Body|Körper/.test(t) && !/🔒/.test(t));
...
await page.evaluate(() => {
  ...
  const b = btns.find((el) => {
    ...
    return /جسم|Body|Körper/.test(t) && !/اختر|Choose|Spiel|Start|رجوع|Back|Zurück/.test(t);
  });
  b?.click();
});
await new Promise((r) => setTimeout(r, 400));
await shot(page, 'koerper-topics.png');
...
await new Promise((r) => setTimeout(r, 500));
await shot(page, 'koerper-picture-match.png');
