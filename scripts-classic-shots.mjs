...
// Select Essen for food art, or Tiere — use Essen to show variety
await page.evaluate(() => {
  const btns = [...document.querySelectorAll('button')];
  const b = btns.find((el) => {
    if (el.disabled) return false;
    const t = el.textContent || '';
    return /طعام|Food|Essen/.test(t) && !/اختر|Choose|Spiel|Start|رجوع|Back|Zurück/.test(t);
  });
  b?.click();
});
...
// Select Classic Cards mode
await page.evaluate(() => {
  const row = document.querySelector('[data-mode="classic"]');
  row?.click();
});
await new Promise((r) => setTimeout(r, 300));
await shot(page, 'classic-modes.png');

await clickText(page, '^ابدأ$|^Start$|^Starten$');
await new Promise((r) => setTimeout(r, 900));
await page.waitForSelector('[data-classic-cards]', { timeout: 5000 });
await page.waitForFunction(() => {
  const img = document.querySelector('[data-card-image]');
  return img && img.complete && img.naturalWidth > 0;
}, { timeout: 8000 }).catch(() => null);
await new Promise((r) => setTimeout(r, 400));
await shot(page, 'classic-front.png');

// Flip card
await page.click('[data-flip-card]');
await new Promise((r) => setTimeout(r, 700));
await shot(page, 'classic-back.png');
